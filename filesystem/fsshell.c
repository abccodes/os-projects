...
int cmd_ls (int argcnt, char *argvec[])
	...
	// no pathname/filename specified - use cwd
	else 
		{
...
int cmd_cat (int argcnt, char *argvec[])
		...
		// Ensure final newline so the prompt starts on its own line
    	printf("\n");
#endif
        return 0;
        }
...
int cmd_mv (int argcnt, char *argvec[])
{
    ...
    // if destArg is a directory, build destPath = destArg + "/" + basename(src)
    ...
    if (fs_isDir(destArg)) {
        ...
        char *bs = strrchr(src, '/');
        const char *name = bs ? bs + 1 : src;

        // assemble new path
        snprintf(destPath, sizeof destPath, "%s/%s", destArg, name);
        destFinal = destPath;
    } else {
        destFinal = destArg;
    }
    ...
    // open (or create) destFinal for writing
    int dst_fd = b_open(destFinal, O_WRONLY | O_CREAT | O_TRUNC);
    if (dst_fd < 0) {
        perror("mv: open dest");
        b_close(src_fd);
        return -1;
    ...
    char buf[BUFFERLEN];
    int n;
    while ((n = b_read(src_fd, buf, BUFFERLEN)) > 0) {
        if (b_write(dst_fd, buf, n) != n) {
            perror("mv: write");
            b_close(src_fd);
            b_close(dst_fd);
            return -1;
...
int main (int argc, char * argv[])
	...
	// // testing stat ----------------------------
	// struct fs_stat st;
	// // Attempt to retrieve metadata for the root directory immediately after initializing the
	// // filesystem
    // if (fs_stat("/", &st) == 0) {
	// // Confirm fs_stat suceeded and display bytes, blocks,block size to verify it is correct
    //     printf("fs_stat on root succeeded:\n");
    //     printf("  size:    %lld bytes\n", (long long)st.st_size);
    //     printf("  blocks:  %lld\n", (long long)st.st_blocks);
    //     printf("  blksize: %d\n",           st.st_blksize);
    // } else {
	// // report any error from fs_stat to know why metadata retrieval failed.
    //     perror("fs_stat");
    // }
	// // testing stat ----------------------------
